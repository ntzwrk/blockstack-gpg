...
parser.add_argument("--disable-fingerprint-length-check", dest="disableLengthCheck", action="store_true", help="disables security length check for the given fingerprint (default: not set, so checks are enabled)")
parser.add_argument("-s", "--silent", dest="silent", action="store_true", help="prints a key on success / nothing on failure (default: not active)")
